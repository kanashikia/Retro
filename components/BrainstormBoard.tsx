      ...
      {Object.values(ColumnType).map((col) => (
        <div key={col} className="flex flex-col gap-6">
          <div className={`flex items-center justify-between p-4 rounded-xl border-b-4 bg-white shadow-sm
            ...
            <button onClick={() => setActiveCol(col)} className="p-2 hover:bg-slate-50 rounded-lg transition-colors border border-transparent hover:border-slate-100">
            ...
            {(session.tickets || [])
              ...
              <div key={ticket.id} className="p-6 rounded-2xl border border-slate-200 bg-white group relative shadow-sm hover:shadow-md transition-shadow animate-in fade-in slide-in-from-bottom-2">
                ...
                <div className="mt-5 flex items-center justify-between">
                  ...
                  {currentUser.isAdmin && <button onClick={() => onUpdateSession({ ...session, tickets: session.tickets.filter(t => t.id !== ticket.id) })} className="opacity-0 group-hover:opacity-100 p-2 text-slate-400 hover:text-red-600 transition-all"><Trash2 className="w-4 h-4" /></button>}
                </div>
              </div>
            ))}
          </div>
